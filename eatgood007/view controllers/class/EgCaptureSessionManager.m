...
@implementation EgCaptureSessionManager
...
- (void)takePicture{
    ...
    [_stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        //delegate 传值
       if ([_delegate respondsToSelector:@selector(didCapturePhoto:withRect:)]) {
            [_delegate didCapturePhoto:croppedImage withRect:self.preview.frame];
        }
    }];
}
//设备朝向的判断
...
- (void)switchGrid:(BOOL)toShow {
    ...
    CGFloat headHeight = _previewLayer.bounds.size.height - [[UIScreen mainScreen] applicationFrame].size.width;
    CGFloat squareLength = [[UIScreen mainScreen] applicationFrame].size.width;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        [self drawALineWithFrame:frame andColor:[UIColor whiteColor] inLayer:_preview.layer];
    }
}
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    if (error != NULL) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"出错了!" message:@"无法保存" delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
        [alert show];
    } else {
        NSLog(@"保存成功!");
    }
